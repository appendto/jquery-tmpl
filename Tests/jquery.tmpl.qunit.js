...
module("Basics");
	...
	test("Newlines / Espcaing", function(){
	...
	test("Caching", function() {
		...
		test_handler( "using a named template", $.render('name', testData)[0].innerHTML, 'name: test' );

		var $elm = $( "<span>name: {{= v }}</span>" );
		$elm.render(testData);
		test_handler( "reusing a template cached in node data", $elm.data("tmpl").call(testData, jQuery, testData, 0), 'name: test' );
	...
	test("Disallowed / Illegal", function(){
		test_handler( "Disallow incremental assignment", R('{{= a += 1 }}', testData), SyntaxError );
		test_handler( "Disallow decremental assignment", R('{{= a -= 1 }}', testData), SyntaxError );
		test_handler( "Disallow multiply assignment", R('{{= a *= 1 }}', testData), SyntaxError );
		test_handler( "Disallow division assignment", R('{{= a /= 1 }}', testData), SyntaxError );
		test_handler( "Disallow left shift assignment", R('{{= a <<= 1 }}', testData), SyntaxError );
		test_handler( "Disallow right shift assignment", R('{{= a >>= 1 }}', testData), SyntaxError );
		test_handler( "Disallow zero-fill right shift assignment", R('{{= a >>>= 1 }}', testData), SyntaxError );
		test_handler( "Disallow bitwise AND assignment", R('{{= a &= 1 }}', testData), SyntaxError );
		test_handler( "Disallow bitwise OR assignment", R('{{= a |= 1 }}', testData), SyntaxError );
		test_handler( "Disallow bitwise XOR assignment", R('{{= a ^= 1 }}', testData), SyntaxError );

		test_handler( "Disallow literal object creation", R('{{= { a:"a"} }}', testData), SyntaxError );
		test_handler( "Disallow literal array creation", R('{{= [1,2,3] }}', testData), SyntaxError );
...
module("Commands");
	...
	test("Each {{ each }}", function() {
		...
		test_handler( "loop", R('{{each arr}}{{= $i }}:{{= this }}/{{/each}}', testData), '0:AA/1:BB/2:CC/' );
		test_handler( "loop", R('{{ each arr as i, item }}{{= i }}:{{= item }}/{{ /each }}', testData), '0:AA/1:BB/2:CC/' );
		test_handler( "loop", R('{{ each arr as i, item }}{{= i }}:{{= this }}/{{ /each }}', testData), '0:AA/1:BB/2:CC/' );
		test_handler( "loop", R('{{ each arr }}{{= $i }}:{{= this }}/{{ /each }}', testData), '0:AA/1:BB/2:CC/' );
	...
	test("With {{ with }}", function() {

		test_handler( "without name does nothing", R('{{ with one }}{{= one }}{{ /with }}', testData), 'first' );
		test_handler( "without name does nothing (2)", R('{{ with one }}{{= two }}{{ /with }}', testData), 'second' );
		test_handler( "new name non-destructively overrides any existing", R('{{ with "whee" as two }}{{= two }}{{ /with }}-{{= two }}', testData), 'whee-second' );
	...
	test("Else {{ else }}", function(){
		
		test_handler( "simple condition check", R('{{ if false }}TRUE{{ else }}FALSE{{ /if }}', { a:1 }), 'FALSE' );
		
		test_handler( 'errors are passed back correctly (syntax)', R("{{ if false }}.{{ else }}{{ syntax_error }}{{ /if }}", testData), SyntaxError );
		test_handler( 'errors are passed back correctly (reference)', R("{{ if false }}.{{ else }}{{ reference_error }}{{ /if }}", testData), ReferenceError );
		test_handler( 'errors are passed back correctly (type)', R("{{ if false }}.{{ else }}{{ type_error }}{{ /if }}", testData), TypeError );		
		
	});
	
	test("If Defined {{ ifdef }}", function(){
		test_handler( "ifdef:undefined", R('{{ ifdef a }}TRUE{{ else }}FALSE{{ /ifdef }}', { }), 'FALSE' );
	
		test_handler( 'errors are passed back correctly (syntax)', R("{{ifdef true}}{{ syntax_error }}{{/ifdef}}", testData), SyntaxError );
		test_handler( 'errors are passed back correctly (reference)', R("{{ifdef true}}{{ reference_error }}{{/ifdef}}", testData), ReferenceError );
		test_handler( 'errors are passed back correctly (type)', R("{{ifdef true}}{{ type_error }}{{/ifdef}}", testData), TypeError );	
	});
	
	test("If Not Defined {{ ifndef }}", function(){
		test_handler( "ifndef:undefined", R('{{ ifndef a }}TRUE{{ else }}FALSE{{ /ifndef }}', { }), 'TRUE' );
	
		test_handler( 'errors are passed back correctly (syntax)', R("{{ifndef a}}{{ syntax_error }}{{/ifndef}}", testData), SyntaxError );
		test_handler( 'errors are passed back correctly (reference)', R("{{ifndef a}}{{ reference_error }}{{/ifndef}}", testData), ReferenceError );
		test_handler( 'errors are passed back correctly (type)', R("{{ifndef a}}{{ type_error }}{{/ifndef}}", testData), TypeError );		
	});
	
	test("Include {{ include }}", function() {
		...
		jQuery.templates['test'] = jQuery.tmpl('{{= $i }}');
		test_handler( "{{each}} index variable", R('{{each arr}}{{ include "test" }}{{/each}}', testData), '012' );

		jQuery.templates['test'] = jQuery.tmpl('{{= n }}');
		test_handler( "{{each}} index variable", R('{{each arr as n,item}}{{ include "test" }}{{/each}}', testData), '012' );

		jQuery.templates['test'] = jQuery.tmpl('{{= item }}');
		test_handler( "{{each}} item variable", R('{{each arr as n,item}}{{ include "test" }}{{/each}}', testData), 'AABBCC' );
